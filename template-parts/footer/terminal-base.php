...
$fb_link   = get_theme_mod( 'gustabe_social_facebook', '#' );
$ig_link   = get_theme_mod( 'gustabe_social_instagram', '#' );
$line_link = gustabe_get_line_url(); // ใช้ helper function จัดการ LINE ID ให้
if( empty($line_link) || $line_link == '#' ) $line_link = $home_url;

$addr  = get_theme_mod('gustabe_contact_address','Address');
$phone = get_theme_mod('gustabe_contact_phone','02-XXX-XXXX');
$email = get_theme_mod('gustabe_contact_email','mail@ex.com');
$hours = get_theme_mod('gustabe_contact_hours','');

$x_link  = get_theme_mod( 'gustabe_social_x', '#' );
$yt_link = get_theme_mod( 'gustabe_social_youtube', '#' );
?>
    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-12 mb-12">
            ...
            <div class="flex flex-col">
                ...
                <div class="text-xs text-slate-400 mb-6 leading-relaxed border-l border-white/10 pl-3 font-mono">
                    System.AI.ready = true;<br>
                    Connected to global network.
                </div>
                <div class="flex gap-4">
                    <?php if ( ! empty($fb_link) && $fb_link !== '#' ) : ?>
                    <a href="<?php echo esc_url($fb_link); ?>" target="_blank" rel="noopener noreferrer" aria-label="Facebook" class="text-slate-400 hover:text-blue-400 hover:-translate-y-1 transition-all duration-300">
                        <i class="huge huge-facebook-02 text-2xl"></i>
                    </a>
                    <?php endif; ?>

                    <?php if ( ! empty($x_link) && $x_link !== '#' ) : ?>
                    <a href="<?php echo esc_url($x_link); ?>" target="_blank" rel="noopener noreferrer" aria-label="X (Twitter)" class="text-slate-400 hover:text-blue-400 hover:-translate-y-1 transition-all duration-300">
                        <i class="huge huge-twitter text-2xl"></i>
                    </a>
                    <?php endif; ?>

                    <?php if ( ! empty($ig_link) && $ig_link !== '#' ) : ?>
                    <a href="<?php echo esc_url($ig_link); ?>" target="_blank" rel="noopener noreferrer" aria-label="Instagram" class="text-slate-400 hover:text-blue-400 hover:-translate-y-1 transition-all duration-300">
                        <i class="huge huge-instagram text-2xl"></i>
                    </a>
                    <?php endif; ?>

                    <?php if ( ! empty($yt_link) && $yt_link !== '#' ) : ?>
                    <a href="<?php echo esc_url($yt_link); ?>" target="_blank" rel="noopener noreferrer" aria-label="YouTube" class="text-slate-400 hover:text-blue-400 hover:-translate-y-1 transition-all duration-300">
                        <i class="huge huge-youtube text-2xl"></i>
                    </a>
                    <?php endif; ?>

                    <?php if ( ! empty($line_link) && $line_link !== '#' ) : ?>
                    <a href="<?php echo esc_url($line_link); ?>" target="_blank" rel="noopener noreferrer" aria-label="Line Chat" class="text-slate-400 hover:text-blue-400 hover:-translate-y-1 transition-all duration-300">
            ...
            <!-- Col 2: Quick Links -->
            <div>
                <h2 class="text-slate-300 text-sm font-semibold mb-6 border-b border-white/10 pb-2">
            ...
            <div>
                ...
                <div class="flex flex-col gap-5 text-sm text-slate-400">
                    <div class="flex items-start gap-3 group">
                        <i class="huge huge-location-01 text-slate-400 group-hover:text-blue-400 transition-colors text-xl mt-0.5"></i>
                        <span class="leading-relaxed"><?php echo nl2br(esc_html(my_pll($addr))); ?></span>
                    </div>
                    <div class="flex items-center gap-3 group">
                        <i class="huge huge-telephone text-slate-400 group-hover:text-blue-400 transition-colors text-xl"></i>
                        <a href="tel:<?php echo esc_attr($phone); ?>" class="hover:text-blue-400 transition-colors"><?php echo esc_html(my_pll($phone)); ?></a>
                    </div>
                    <div class="flex items-center gap-3 group">
                        <i class="huge huge-mail-02 text-slate-400 group-hover:text-blue-400 transition-colors text-xl"></i>
                        <a href="mailto:<?php echo esc_attr($email); ?>" class="hover:text-blue-400 transition-colors"><?php echo esc_html(my_pll($email)); ?></a>
                    </div>
                    <?php if ( ! empty($hours) ) : ?>
                    <div class="flex items-center gap-3 group">
                        <i class="huge huge-time-02 text-slate-400 group-hover:text-blue-400 transition-colors text-xl"></i>
                        <span class="leading-relaxed"><?php echo esc_html(my_pll($hours)); ?></span>
                    </div>
                    <?php endif; ?>
                </div>
            </div>
        ...
        <!-- 🛡️ Copyright Bar -->
        <div class="border-t border-white/10 pt-6 flex flex-col md:flex-row justify-between items-center text-[11px] text-slate-400 font-mono">
