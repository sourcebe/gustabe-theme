				...
				<tr class="<?php echo esc_attr( apply_filters( 'woocommerce_cart_item_class', 'cart_item', $cart_item, $cart_item_key ) ); ?>">
					<td class="product-name">
						<div class="flex items-center gap-4">
							<div class="product-thumbnail border border-emerald-400/50 rounded-md overflow-hidden bg-slate-900/50 flex-shrink-0 w-16 h-16 flex items-center justify-center">
								<?php 
								$thumbnail = $_product->get_image( array( 64, 64 ) );
								if ( $thumbnail ) {
									echo $thumbnail;
								} else {
									echo '<i class="huge huge-image-01 text-slate-600 text-2xl"></i>';
								}
								?>
							</div>
